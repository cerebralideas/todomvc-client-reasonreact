...
let getFilter = () => {
	let lsValue = Dom.Storage.(localStorage |> getItem("filter"));
	switch (lsValue) {
		| None => "all"
		| Some(x) => x
	}
...
let getTodos = () => {
	...
	let stringifiedTodos = switch (lsValue) {
		| None => "[]"
		| Some(x) => x
	};
	...
	stringifiedTodos |>
		Json.parseOrRaise |>
		json => Json.Decode.( json |> array(parseTodos)) |>
		Array.to_list;
};
let setFilter = (filter: string) => {
	Dom.Storage.(localStorage |> setItem("filter", filter));
};
let setTodos = (todos: list(StateTypes.todo)) => {
	let stringifiedTodos = todos |>
		Array.of_list |>
